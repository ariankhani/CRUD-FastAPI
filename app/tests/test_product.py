...
def created_item(test_client, auth_token):
    ...
    data = {"name": "test_item", "price": "10.0"}
    ...
    print("Create error details:", response.json())
...
def test_create_product(test_client, auth_token):
    ...
    # Supply a dummy file (empty is fine since validations are overridden)
    dummy_file = io.BytesIO(b"")
    dummy_file.name = "dummy.jpg"
    files = {"image": ("dummy.jpg", dummy_file, "image/jpeg")}
    response = test_client.post(
...
def test_read_item(test_client, auth_token, created_item):
    headers = {"Authorization": f"Bearer {auth_token}"}
    response = test_client.get(f"/products/{created_item}", headers=headers)
    print("Read error details:", response.json())
...
def test_update_item(test_client, auth_token, created_item):
    headers = {"Authorization": f"Bearer {auth_token}"}
    data = {"name": "updated_item", "price": "12.0"}
    response = test_client.put(
        f"/products/update/{created_item}", data=data, headers=headers
    )
    print("Update error details:", response.json())
    assert response.status_code == 200, (
        f"Unexpected status code: {response.status_code}"
    )
...
def test_delete_item(test_client, auth_token, created_item):
    ...
    print("Delete error details:", response.json())
    assert response.status_code == 200, (
        f"Unexpected status code: {response.status_code}"
    )
